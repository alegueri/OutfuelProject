...
/**
 * The preprocessDataSets class offers two private preprocessing methods for the data set of vehicles and
 * the data set of gas stations. These preprocessing methods organize and structure the data sets before
 * runtime. These method are to be run before runtime so data read is preprocessed.
 * 
 * @author Max Mercer
 *
 */
...
public class preprocessDataSets {
	...
	/**
	 * This routine reads from the original data set titled GasStationsDATA and writes to the new data set
	 * preprocessedGasStationsDATA.
	 * 
	 * The preprocessed data will not contain duplicate stations. The type of gas a station serves will be added.
	 * The latitude and longitude of the stations location is added. And data is parsed so its format is acceptable
	 * by the load class.
	 * 
	 * @throws Exception
	 */
	private static void preprocessGasStations() throws Exception{
        ...
        for (int i = 0 ; i < stationArray.length-numberRemoved ; i++){
            ...
            String latLongs[] = GoogleLATLONG.getLatLongPositions(currentStation.getFullAddress());
            ...
            writer.write(currentStation.getStationName() + ", " + currentStation.getAddress() + ", " + currentStation.getSuburb() + ", " + averagePrice + ", " + gasTypes + ", " + rewards + ", " + latLongs[0] + ", " + latLongs[1] + "\n");
        }
        input.close();
        writer.close();
	}
	...
	/**
	 * This routine reads from the original data set titled VehiclesDATA and writes to the new data set
	 * preprocessedVehiclesDATA.
	 * 
	 * The preprocessed data will not contain duplicate vehicles and all vehicles of the same year make and model
	 * will will have one line for that vehicle with the vehicles MPG (miles per gallon) averaged out among the 
	 * duplicate vehilces.
	 * 
	 * Data is parsed so its format is acceptable by the load class.
	 * 
	 * @throws Exception
	 */
